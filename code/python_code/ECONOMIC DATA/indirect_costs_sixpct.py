# -*- coding: utf-8 -*-
"""
indirect_costs_sixpct.py: Calculates indirect costs of COVID infection.

Please refer to indirect_costs.py for detailed comments. Note that the only
difference is that we use a 6% discount rate here instead of 3%.
...
wdl_df["critical"] = wdl_df["critical_nopasc"] + prop_pasc*((365.25-wdl_df["critical_nopasc"]) + (wdl_df["critical_nopasc"] + (365.25/2))*((1.06)**(-1)))
...
udl_df["critical"] = udl_df["critical_nopasc"] + prop_pasc*((365.25-udl_df["critical_nopasc"]) + (udl_df["critical_nopasc"] + (365.25/2))*((1.06)**(-1)))
...
indirect_costs.to_excel("../output/indirect_costs_jk_sixpct.xlsx", index = False)
