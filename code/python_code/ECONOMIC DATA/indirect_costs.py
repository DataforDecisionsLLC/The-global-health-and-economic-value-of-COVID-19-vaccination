# -*- coding: utf-8 -*-
"""
indirect_costs.py: Calculates indirect costs of COVID infection.
...
inputs:
    ...
    - daily_earn_2019USD.dta (processed file)
...
lifetables2019 = pd.read_excel("../input/WPP2022_POP_F01_1_POPULATION_SINGLE_AGE_BOTH_SEXES-import.xlsx",
                           sheet_name = "import_2", header = 1)

# keep country and age-specific columns
lifetables2019 = lifetables2019[["country"] + [col for col in lifetables2019.columns if "age_" in col]]
...
wdl_df["critical"] = wdl_df["critical_nopasc"] + prop_pasc*((365.25-wdl_df["critical_nopasc"]) + (wdl_df["critical_nopasc"] + (365.25/2))*((1.03)**(-1)))
...
# create a dictionary with age-specific rates of IMV for severe infections
imv_severe_rate = dict()
imv_severe_rate["15_17"] = 0.009
imv_severe_rate["18_29"] = 0.014
imv_severe_rate["30_49"] = 0.032
...
# create a dictionary with age-specific rates of IMV for critical infections
imv_critical_rate = dict()
imv_critical_rate["15_17"] = 0.276 
imv_critical_rate["18_29"] = 0.391
imv_critical_rate["30_49"] = 0.500
...
# calculate unpaid workdays lost for severe, critical, and mild cases 
udl_severe = dict()
for key, value in imv_severe_rate.items():
...
# create a list of ages 15 - 100
unpaid_ages = list()
for age in range(15,101):
    age_col = "age_" + str(age)
    unpaid_ages.append(age_col)
    
# create a data frame with only countries and unpaid work ages
lifetables_unpaid = lifetables2019[["country"] + unpaid_ages]
# calculate the total population in these ages
lifetables_unpaid["working_pop"] = lifetables_unpaid[unpaid_ages].sum(axis = 1)
# calculate the proportion of the pouplation in each age group
...
# calculate the average number of unpaid work days lost for mild, severe, and critical infections
...
# add long covid to critical infections
udl_df["critical"] = udl_df["critical_nopasc"] + prop_pasc*((365.25-udl_df["critical_nopasc"]) + (udl_df["critical_nopasc"] + (365.25/2))*((1.03)**(-1)))

# drop the non-long COVID critical field
udl_df = udl_df.drop(columns = "critical_nopasc")
...
# add unpaid work hours per day to the unpaid work loss from covid data frame
unpaid_df = udl_df.merge(unpaid_time, on = "country", how = "left")
# add wages too
unpaid_df = unpaid_df.merge(wages_df, on = "country", how = "left")
# calculate cost per day as the product of the hourly wage and hours of unpaid work per day
unpaid_df["cost_per_day"] = unpaid_df["wage"]*unpaid_df["hours"]

# calculate unpaid work cost for mild, severe, and critical infections
...
# add severity splits to the data
unpaid_disability = disability.merge(unpaid_df[["country", "mild_cost", "severe_cost", "critical_cost"]],
                              on = "country", how = "left")
# assign an unpaid work loss of zero for asymptomatic infections
unpaid_disability["asymp_cost"] = 0

# calculate the average nonfatal unpaid work cost
...
# calculate the yearly hours of unpaid work
unpaid_df["yearly_hours"] = unpaid_df["hours"]*365.25
# multiply that by hourly wage to get the annual value of unpaid work
unpaid_df["annual_unpaid_value"] = unpaid_df["yearly_hours"]*unpaid_df["wage"]

# consider only the 2019 lifetable and rename countries
...
# if the age is less than 15, set the annual value to zero
...
# calculate unpaid work loss from mortality
def unpaidwork_loss_mortality(country, age, discount_rate):
    """
    Calculates the unpaid work loss from mortality.
...
# calculate population-weighted unpaid work costs in age groups
def fatal_costs_age_specific(country, first_age, last_age):
    global age_specific_df
    """
...
# calculate the total unpaid work loss
fatal_df["fatal_unpaid_work_loss"] = fatal_df.filter(regex = "^unpaid_work_loss_").sum(axis = 1)

# combine fatal and nonfatal unpaid work losses into a single data frame
...
# combine paid work loss and unpaid work loss to a single data frame
...
# calculate the total nonfatal and fatal indirect cost
...
# save to Excel
indirect_costs.to_excel("../output/indirect_costs_jk.xlsx", index = False)
