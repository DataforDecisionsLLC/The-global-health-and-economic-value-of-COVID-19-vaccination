...
use "$output\ECONOMIC DATA\INDIRECT COSTS\unpaid_work_loss_nonfatal_case_6pct_discount_rate", clear
ren unpaid_work_loss_nonfatal_case indirect_costs_nf_case
drop currency 
compress 
save "$output\ECONOMIC DATA\INDIRECT COSTS\indirect_costs_nonfatal_case_6pct_discount_rate", replace
...
merge 1:1 country year quarter yyyy_qq using "$output\ECONOMIC DATA\INDIRECT COSTS\indirect_costs_nonfatal_case_6pct_discount_rate"
drop if year==2022 
keep if _m==3 
drop _m 
...
merge 1:1 country year quarter yyyy_qq using "$output\QALY\6pct_discounted_qaly_losses_per_fatal_case", ///
...
compress 
save "$root\VALUE OF VAX\value_of_vax_costs_6pct_discounted", replace
...
append using  "$root\VALUE OF VAX\value_of_vax_costs_6pct_discounted"
...
merge 1:1 country yyyy_qq year quarter brand using "$root\VALUE OF VAX\value_of_vax_costs_6pct_discounted"
assert _m!=1
drop _m 
