...
merge 1:m country_group using "$root\REGRESSION RESULTS\SUR\LOWER UPPER INF\sim_0_novax_inf_all_vax_lower"
assert _m!=1 
keep if _m==3 
drop _m 
drop country_group 
...
save "$root\REGRESSION RESULTS\ANNUAL & QUARTERLY GDP\sim_0_novax_inf_all_vax_quarterly_gdp_lower_overall", replace
...
merge 1:m country using "$root\REGRESSION RESULTS\ANNUAL & QUARTERLY GDP\sur_regbase_infections_lower_gdp_qrtly"
assert _m!=2
keep if _m==3
drop _m 
...
merge 1:1 country time using "$root\REGRESSION RESULTS\ANNUAL & QUARTERLY GDP\sim_0_novax_inf_all_vax_quarterly_gdp_lower_overall"
drop if _m==2
...
save "$root\REGRESSION RESULTS\SUR\LOWER UPPER INF\lower_gdp_pfizer_v_other_overall", replace
