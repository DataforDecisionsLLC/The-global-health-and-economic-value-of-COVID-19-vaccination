...
capture log close
log using "$root\REGRESSION RESULTS\SUR\make_X_vax_pfz", text replace
...
sort variable time 
compress
save "$root\REGRESSION RESULTS\SUR\X_vax_pfz.dta",replace

end

getshell 135
use "$root\REGRESSION RESULTS\SUR\X_vax_pfz.dta", clear
drop x_135
save "$root\REGRESSION RESULTS\SUR\X_vax_pfz.dta", replace
...
merge 1:1 variable time using "$root\REGRESSION RESULTS\SUR\X_vax_pfz.dta"
assert _m!=1
