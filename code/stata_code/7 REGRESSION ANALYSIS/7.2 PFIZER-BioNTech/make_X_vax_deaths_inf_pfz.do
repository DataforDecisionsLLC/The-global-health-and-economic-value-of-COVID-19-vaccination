...
capture log close
log using "$root\REGRESSION RESULTS\SUR\make_X_vax_deaths_inf_pfz", text replace
...
sort variable time 
compress
save "$root\REGRESSION RESULTS\SUR\X_vax_deaths_inf_pfz.dta",replace

end

getshell 138
use "$root\REGRESSION RESULTS\SUR\X_vax_deaths_inf_pfz.dta", clear
drop x_138
save "$root\REGRESSION RESULTS\SUR\X_vax_deaths_inf_pfz.dta", replace
...
merge 1:1 variable time using "$root\REGRESSION RESULTS\SUR\X_vax_deaths_inf_pfz.dta"
assert _m!=1
