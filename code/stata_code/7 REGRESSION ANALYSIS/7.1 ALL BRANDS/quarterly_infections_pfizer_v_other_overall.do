...
capture log close
log using "$root\REGRESSION RESULTS\SUR\no_cont_2_lags_infections_pfizer_v_other_overall", text replace
...
testparm time_7-time_13
local fstat_time = r(F)
local pval_time = r(p)

testparm country_2-country_141
local fstat_cty = r(F)
local pval_cty = r(p)
...
tot_pop yhat_1 yhat_2 inf_mean 
...
gen pandemic_inf = inf_mean *tot_pop 

save "$root\REGRESSION RESULTS\SUR\no_cont_2_lags_infections_pfizer_v_other_overall", replace
